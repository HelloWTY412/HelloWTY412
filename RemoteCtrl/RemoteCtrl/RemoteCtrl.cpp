...
#include"CSocket.h"
#include"CNetwork.h"
int RecvFromCB(void* arg, const CBuffer& buffer, CSockaddrIn& addr) {
    EServer* server = (EServer*)arg;
    return server->Sendto(addr,buffer);
}
int SendToCB(void* arg, CSockaddrIn& addr, int ret) {
    EServer* server = (EServer*)arg;
    printf("sendto done\r\n");
    return 0;
}
void udp_server() {
    list<CSockaddrIn> lstclients;
    printf("%s(%d):%s\r\n", __FILE__, __LINE__, __FUNCTION__);
    CServerParamter param("127.0.0.1",20000,ETYPE::ETypeUDP,NULL,NULL,NULL,RecvFromCB,SendToCB);
